...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, MobileNetV3Small, MobileNetV3Large, SwinTransformerTiny
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if args.zca:
        ...
        for i in tqdm.tqdm(range(len(dst_train))):
            ...
            images.append(im)
            labels.append(lab)
        # forcer ZCA sur CPU car SVD pas supporte sur MPS
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True):
    ...
    elif model == 'MobileNetV3Small' or model == 'MobileNetV3':
        net = MobileNetV3Small(channel=channel, num_classes=num_classes)
    elif model == 'MobileNetV3Large':
        net = MobileNetV3Large(channel=channel, num_classes=num_classes)
    elif model == 'SwinTransformer' or model == 'SwinTransformerTiny':
        net = SwinTransformerTiny(channel=channel, num_classes=num_classes)
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M': # multiple architectures
        ...
        model_eval_pool = ['ConvNet', 'AlexNet', 'VGG11', 'ResNet18_AP', 'ResNet18', 'MobileNetV3Small', 'SwinTransformerTiny']
